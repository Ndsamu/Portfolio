...
class Stack:
    ...
    def __init__(self, decktype, deckid, x, y, game=None):
        """Stack constructor.
    ...
    def doubleclickhandler(self, event):
        print "Double Click Event Handler"
        self.finishmoving()             # In case we lost an event
...
class OpenStack(Stack):
    ...
    def usermovehandler(self, cards):
        print "OpenStack Move"
        card = cards[0]
        # Selects the stack your mouse is over
        ...
        if not stack or stack is self or not stack.acceptable(cards):
            ...
            for card in cards:
                # Controls deletion of top card after being moved
                print "UserMoveHandler Card: ", card
                self.delete(card)
                stack.add(card)
            self.game.wincheck()

    def userdoubleclickhandler(self):
        print "OpenStack Double"
        if not self.cards:
            return
        card = self.cards[-1]
        if not card.face_shown:
...
class Solitaire:
    ...
    def __init__(self, master, agent):
        ...
        x = MARGIN
        y = MARGIN
        self.deck = Deck("Deck", 1, x, y, self)

        x = x + XSPACING
        self.opendeck = OpenStack("Talon", 1, x, y, self)

        x = x + XSPACING
        self.suits = []
        for i in range(NSUITS):
            x = x + XSPACING
            self.suits.append(SuitStack("Suit", i+1, x, y, self))
        ...
        for i in range(NROWS):
            self.rows.append(RowStack("Build", i+1, x, y, self))
            x = x + XSPACING
        ...
        for i in range(0, trials):
            ...
            print "[", float(i+1)/float(trials)*100, "%]"
            print "Result: ", result
            self.agent.reset()
        success = float(outcomes.count(True))
        print "Success Rate: ", success/float(trials) * 100.0, "%"

    def activecards(self):
        active = []
        openstacks = [self.opendeck] + self.suits + self.rows
        visiblechunk = []
        for stack in openstacks:
            for card in stack.cards:
                if len(stack.cards) > 0:
                    if card.face_shown:
                        active.append([stack, card])
        return active
    ...
    def gameplay(self):
        ...
        self.deck.add(card)
        card.showface()
        self.deck.userclickhandler()
        failure = 0
        movect = 0
        while not self.wincheck():
            move = self.agent.play(self.opendeck, self.rows, self.suits)
            for build in self.rows:
                if len(build.cards) > 0 and not build.cards[-1].face_shown:
                    build.showtop
            if not (move is None):
                self.animatedmoveto(move.card, move.dest)
                #print "Move Count: ", movect
                if move.card == move.source.cards[-1]:
                    movect += 1
                    move.dest.add(move.card)
                    move.source.delete(move.card)
                    move.source.showtop()
                else:
                    source = list(move.source.cards)
                    flag = False
                    for card in source:
                        if card.face_shown:
                            #print "Card's Face Shown: ", card
                            if card == move.card:
                                flag = True
                            if flag:
                                movect += 1
                                move.dest.add(card)
                                move.source.delete(card)
                    if len(move.source.cards) > 0:
                        if not move.source.cards[-1].face_shown:
                            move.source.showtop()

            else:
                if failure < 300:
                    self.deck.userclickhandler()
                    #print "Unplayable Hands: ", failure
                    failure += 1
                else:
                    #print "Agent Failed"
                    return False
