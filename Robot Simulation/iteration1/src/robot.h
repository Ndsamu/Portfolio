...
class Robot : public ArenaMobileEntity {
 public:
  ...
  int get_captured_bases();

  void set_captured_bases(int c);
 ...
 private:
  ...
  int lives_;
  // Captured bases are incremented as the robot collides with them.
  // When all bases are captured, the game is won.
  int captured_;
  // Mercy Switch to turn on or off invincibility
  bool mercy_switch_;
  // Mercy Timer to count how long the mercy timer should run
  int mercy_timer_;
  // Used to toggle robot color for flashing effect
  bool mercy_flash_;
};
