...
Arena::Arena(const struct arena_params *const params)
  ...
  AddObstacle(6);  // params->n_obstacles
}
...
void Arena::UpdateEntitiesTimestep() {
   ...
   * Check for win/loss
   */
  if (robot_->get_captured_bases() == N_BASES) {
    game_status_ = WON;
    robot_->set_color(ROBOT_COLOR_WON);
  } else if (robot_->get_lives() == 0) {
    game_status_ = LOST;
  ...
  for (auto &ent1 : mobile_entities_) {
    ...
    if (kUndefined != wall) {
      ...
      if (ent1->get_type() == kObstacle) {
        obstacle_ = dynamic_cast<Obstacle *>(ent1);
        obstacle_->HandleCollision();
      } else {
      ...
      if (IsColliding(ent1, ent2)) {
        ...
        if (ent1->get_type() == kObstacle) {
          ...
          if (ent2->get_type() == kBase) {
            base_ = dynamic_cast<Base *>(ent2);
            if (!base_->IsCaptured()) {
              robot_->HandleCollision(ent2->get_type(), ent2);
              base_->set_captured(true);
              base_->set_color(BASE_COLOR_TAKEN);
            }
          } else {
            robot_->HandleCollision(ent2->get_type(), ent2);
          }
...
void Arena::AdjustEntityOverlap(ArenaMobileEntity * const mobile_e,
  ArenaEntity *const other_e) {
    ...
    double distance_to_move =
      mobile_e->get_radius() + other_e->get_radius() - distance_between + 10;
